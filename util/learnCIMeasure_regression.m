function [measure,initialMeasure, Analysis] = learnCIMeasure_regression(InputBags, InputLabels, Parameters,trueInitMeasure)
%% learnCIMeasure_regression()
% This function learns a fuzzy measure for Multiple Instance Choquet Integral Regression (MICIR) algorithm
% This function works with regression problems (real-valued labels!)
%       as well as classification problems. Note that in two-class
%       classification, the negative bags HAVE TO be one point per bag.
% This function uses a min(sum(min(*))) objective function and an evolutionary algorithm to optimize.
%
% INPUT
...
if nargin == 4 %with initialMeasure input
    parfor i = 1:Parameters.nPop
        ...
        fitnessPop(i) = evalFitness_reg(Labels, measurePop(i,:), nPntsBags, oneV, bag_row_ids, diffM);
    end
...
for iter = 1:Parameters.maxIterations
    ...
    parfor i = 1:Parameters.nPop
        ...
        if(z < eta) %Small-scale mutation: update only one element of the measure
            ...
            childFitness(i) = evalFitness_reg(Labels, childMeasure(i,:), nPntsBags, oneV, bag_row_ids, diffM);
            JumpType(i) = 1;
